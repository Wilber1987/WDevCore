...
class WModalForm extends HTMLElement {
	...
	constructor(Config) {
		...
		this.FullScreen = this.Config.FullScreen ?? false;
			...
			props: {
				ClassList: [
					new WCssClass(".ModalContentWModal", {
						...
						"overflow-y": "auto",
						"padding-bottom":this.FullScreen == "FullScreen" ? 0 : 50
					}), new WCssClass(this.FullScreen ? ".ContainerFormWModal": "undefidedStyle", {
						"width": "100% !important",
					})
				], MediaQuery: [{
	...
	DrawComponent = async () => {
		...
		this.Modal = {
			class: "ContainerFormWModal" + (this.FullScreen ? " ContainerFormWModalFullScreen" : ""),
			style: {
